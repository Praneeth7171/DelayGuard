...
```
backend/
  app.py                 Flask routes: login/logout, /api/requests, Excel upload
  models.py              User + Request models, the SLA risk-scoring formula
  seed.py                Run once: creates the 3 logins + loads requests_template.xlsx
  make_template.py       Generates requests_template.xlsx (edit SAMPLE_ROWS to change starting data)
  requirements.txt
  templates/index.html   The dashboard UI — calls the API, no fake/hardcoded data
```